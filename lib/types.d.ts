...
declare type TOriginTypeMap<T> = {
    [p in keyof T]: T[p] extends any[] ? UnionToIntersection<React.ComponentProps<T[p][0]>> : React.ComponentProps<T[p]>;
};
declare type UnionToIntersection<U> = (U extends any ? (k: U) => void : never) extends (k: infer I) => void ? I : never;
export declare type IFormItem<T> = TFormItems<T>[keyof TFormItems<T>];
